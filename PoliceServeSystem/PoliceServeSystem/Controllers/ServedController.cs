...
{    
    public class ServedController : Controller
    {
        ...
        public JsonResult SaveSign(string sign, string CaseN, string SignMode, string AccusedNo, string OffenseNo)
        {

            System.Drawing.Bitmap signatureImage = (System.Drawing.Bitmap)SigJsonToImage(sign);
            bool print2PDFFormat = false;//, //printWaterMarkCopy = false, preview = false;
            string printMode;

            AccessController.CurrentAccusedNo = AccusedNo;
            AccessController.CurrentOffenseNo = OffenseNo;
            string currentfilename = DateTime.Now.Ticks.ToString() + ".jpg";
            string caseNo = string.Empty;
            string signType = string.Empty;
            int signMode = Convert.ToInt16(SignMode);
            ...
            bool flag = false;
            SignatureBase_DAL signaturedalfile = new SignatureBase_DAL();

            flag = signaturedalfile.Net_InsertSignature(signature); // For SignMode = 2
            ...
            if (flag == true)
            {
                if (Convert.ToString(AccessController.CurrentUserRole).ToUpper() == "JUDGE" || Convert.ToString(AccessController.CurrentUserRole).ToUpper() == "JAILER")// this is used for judge module for save judge user id for display for special condition and orderemergency report
                {
                    //signature.UserID = (string)Session["UserID"];
                    //signature.saveSignatureToDatabaseWithUserID();
                    SaveLoggInfo("singture is updated for case no " + caseNo + " and sign mode is " + signature.Signmode, Convert.ToInt16(LoggerBase.LoggerType.Edit));
                }
            }

            string signLogText = Convert.ToString(CommonRoutines.DBServerDateTime) + " Sign " + caseNo + " " + signType + "\n";
            StatisticLogger.writeLog(signLogText);
